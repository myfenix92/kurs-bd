...
export const ModelTablePage = class {
	...
	changeTableName(id_table, new_name_table) {
		API
			.changeNameTable(id_table, new_name_table)
			.then(data => {
				if (data.message) {
					...
					this
						.nameTable
						.setAttribute('placeholder', data.message);
				}
			});
	}
	...
	filterByAlphabet(id_sticker, i) {
		API
			.getFilterRecordAlphabet(id_sticker)
			.then(data => {
				data.forEach((e) => {
					ViewTP.viewRecords(e.id_record, e.record, i, e.done);
				});
	...
	filterByDone(id_sticker, i) {
		API
			.getFilterRecordDone(id_sticker)
			.then(data => {
				data.forEach((e) => {
					ViewTP.viewRecords(e.id_record, e.record, i, e.done);
				});
	...
	filterByNotDone(id_sticker, i) {
		API
			.getFilterRecordNotDone(id_sticker)
			.then(data => {
				data.forEach((e) => {
					ViewTP.viewRecords(e.id_record, e.record, i, e.done);
				});
	...
	getBgImage(page, query = 'morning') {
		...
		unsplash
			.search
			.getPhotos({query: query, page: page, perPage: 10, orientation: 'landscape'})
			.then(data => {
				if (data.response.total_pages === 0) {
					document
						.querySelector('.bg_image_error')
