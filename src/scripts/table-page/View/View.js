...
export const ViewTablePage = class {
	...
	viewStickers(id_sticker, nameSticker) {
		...
		this
			.column
			.setAttribute('data-id_sticker', id_sticker);
		...
		this.liArray = [
			...
			'Сортировать по дате создания (новые)',
			'Сортировать по дате создания (старые)',
			'Сортировать по выполнению (выполненные)',
			'Сортировать по выполнению (невыполненные)',
			'Удалить стикер'
		];
		this.liArrayClass = [
			'sort_alphabet',
		...
		this
			.headerPopMenu
			.append(this.headerPopMenuTitle);
		this
			.popMenu
			.append(this.headerPopMenu, this.hr, this.listPopMenu);
		this
			.btnMenuColumn
			.append(this.imgBtnMenu);
		this
			.titleColumn
			.append(this.titleSpan, this.titleColumnInput, this.btnMenuColumn);
		this
			.column
			.append(this.titleColumn, this.popMenu, this.columnList, this.btnAddRec);
		this
			.main
			.append(this.column);
	}
	...
	viewRecords(id_record, recordText, i, done, order_user) {
		...
		if (done) {
			this
				.isDone
				.setAttribute('checked', done);
		}
		this
			.isDone
			.setAttribute('name', 'done');
		this
			.record
			.classList
			.add('show');
		...
		this
			.record
			.setAttribute('data-id_record', id_record);
		this
			.record
			.setAttribute('id', id_record);
		this
			.record
		...
		this
			.record
			.append(this.isDone, this.recordLabel, this.recordTextInput, this.recordText);
	...
	viewCreateStickers() {
		...
		this
			.titleColumnInput
			.classList
			.add('title_column_input', 'new_sticker');
		...
		this.liArray = ['Сортировать по алфавиту', 'Сортировать по дате создания (новые)', 'Сортировать по дате создания (старые)', 'Удалить стикер'];

		this.liArrayClass = ['sort_alphabet', 'sort_new', 'sort_old', 'delete_sticker'];
	...
	viewHistoryChanges() {
		...
		this
			.historyBlock
			.append(this.titleHistoryBlock, this.historyList);
		this
			.overlay
			.appendChild(this.historyBlock);
		this
			.main
	...
	viewBgImageChanges() {
		...
		this
			.bgImageList
			.appendChild(this.btnLoadMore);

		this
			.overlay
			.append(
				this.titleBgImageBlock,
				this.bgImageBtn,
				this.bgColorBtn,
				this.br,
				this.bgColorInput,
	...
	viewColorBlock() {
		...
		for (let i = 0; i < this.colorArray.length; i++) {
			...
			this
				.bgColorBlock
				.classList
				.add('color', `color_${this.colorArray[i]}`);
			this.bgColorBlock.style.background = this.colorArray[i];
			this
				.bgImageList
				.appendChild(this.bgColorBlock);
		}
	}
